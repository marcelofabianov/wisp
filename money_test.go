...
func (s *MoneySuite) TestNewMoney() {
	s.Run("should create a new money object successfully", func() {
		m, err := atomic.NewMoney(1050, atomic.BRL)
		s.Require().NoError(err)
		s.False(m.IsZero())
		s.Equal(int64(1050), m.Amount())
		s.Equal(atomic.BRL, m.Currency())
	})

	s.Run("should fail with an invalid currency", func() {
		m, err := atomic.NewMoney(1000, atomic.Currency("XYZ"))
		s.Require().Error(err)
		s.True(m.IsZero())
	...
	s.Run("should fail with a zero currency", func() {
		m, err := atomic.NewMoney(1000, atomic.EmptyCurrency)
		s.Require().Error(err)
		s.True(m.IsZero())
	})
}
...
func (s *MoneySuite) TestMoney_Comparison() {
	brl100, _ := atomic.NewMoney(10000, atomic.BRL)
	brl200, _ := atomic.NewMoney(20000, atomic.BRL)
	usd100, _ := atomic.NewMoney(10000, atomic.USD)

	s.Run("Equals", func() {
		brl100Clone, _ := atomic.NewMoney(10000, atomic.BRL)
		s.True(brl100.Equals(brl100Clone))
		s.False(brl100.Equals(brl200))
		s.False(brl100.Equals(usd100))
		s.False(brl100.Equals(atomic.ZeroMoney))
	})
...
func (s *MoneySuite) TestMoney_Arithmetic() {
	brl100, _ := atomic.NewMoney(10000, atomic.BRL)
	brl50, _ := atomic.NewMoney(5000, atomic.BRL)
	usd50, _ := atomic.NewMoney(5000, atomic.USD)
...
func (s *MoneySuite) TestMoney_Split() {
	s.Run("should split an even amount correctly", func() {
		m, _ := atomic.NewMoney(10000, atomic.BRL)
		parts, err := m.Split(4)
		s.Require().NoError(err)
		s.Len(parts, 4)
	...
	s.Run("should split an uneven amount distributing the remainder", func() {
		m, _ := atomic.NewMoney(10000, atomic.BRL) // 100.00
		parts, err := m.Split(3)
		s.Require().NoError(err)
		s.Len(parts, 3)
...
func (s *MoneySuite) TestMoney_JSONMarshaling() {
	s.Run("should marshal and unmarshal correctly", func() {
		m, _ := atomic.NewMoney(12345, atomic.EUR)
		...
		var unmarshaledMoney atomic.Money
...
func (s *MoneySuite) TestMoney_Representation() {
	m, _ := atomic.NewMoney(9990, atomic.USD)
