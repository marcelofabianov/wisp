...
func (s *UFSuite) TestNewUF() {
	testCases := []struct {
		...
		{name: "should create an empty UF from an empty string", input: "", expected: wisp.EmptyUF},
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			uf, err := wisp.NewUF(tc.input)
			if tc.expectError {
				s.Require().Error(err)
				s.Equal(wisp.EmptyUF, uf)
...
func (s *UFSuite) TestUF_IsValidAndZero() {
	...
	s.True(sp.IsValid())
	s.False(xx.IsValid())
	s.False(wisp.EmptyUF.IsValid())

	s.False(sp.IsZero())
	s.True(wisp.EmptyUF.IsZero())
}
...
func (s *UFSuite) TestUF_DatabaseInterface() {
	...
	s.Run("Scan", func() {
		s.Run("should scan a valid string", func() {
			var scannedUF wisp.UF
			err := scannedUF.Scan("SC")
			s.Require().NoError(err)
			s.Equal(wisp.UF("SC"), scannedUF)
		})
