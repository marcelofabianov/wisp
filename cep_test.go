...
func (s *CEPSuite) TestNewCEP() {
	testCases := []struct {
		...
		{name: "should create an empty CEP from an empty string", input: "", expected: wisp.EmptyCEP},
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			cep, err := wisp.NewCEP(tc.input)
			if tc.expectError {
				s.Require().Error(err)
				s.Equal(wisp.EmptyCEP, cep)
...
func (s *CEPSuite) TestCEP_Methods() {
	cep, _ := wisp.NewCEP("74835030")

	s.Run("IsZero", func() {
		s.False(cep.IsZero())
		s.True(wisp.EmptyCEP.IsZero())
	})
...
func (s *CEPSuite) TestCEP_DatabaseInterface() {
	...
	s.Run("Value", func() {
		...
		nilVal, err := wisp.EmptyCEP.Value()
		s.Require().NoError(err)
		s.Nil(nilVal)
	})
	...
	s.Run("Scan", func() {
		s.Run("should scan a valid string", func() {
			var scannedCEP wisp.CEP
			err := scannedCEP.Scan("12345678")
			s.Require().NoError(err)
			s.Equal(wisp.CEP("12345678"), scannedCEP)
		})
