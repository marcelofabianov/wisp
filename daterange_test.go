...
func (s *DateRangeSuite) TestNewDateRange() {
	...
	s.Run("should create a valid single-day range", func() {
		dr, err := wisp.NewDateRange(start, start)
		s.Require().NoError(err)
		s.Equal(1, dr.Days())
	})

	s.Run("should fail if start date is after end date", func() {
		_, err := wisp.NewDateRange(end, start)
		s.Require().Error(err)
	})
}

func (s *DateRangeSuite) TestDateRange_Methods() {
	start, _ := wisp.NewDate(2025, 9, 10)
	end, _ := wisp.NewDate(2025, 9, 20)
	dr, _ := wisp.NewDateRange(start, end)

	s.Run("Contains", func() {
		inside, _ := wisp.NewDate(2025, 9, 15)
		before, _ := wisp.NewDate(2025, 9, 9)
...
func (s *DateRangeSuite) TestDateRange_JSONMarshaling() {
	...
	s.Run("should marshal and unmarshal correctly", func() {
		...
		var unmarshaledDR wisp.DateRange
